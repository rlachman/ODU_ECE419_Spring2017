...
class DataEncryptionStandard {
    ...
    private static byte[] unwrap(String password,int size) {
        byte[] ident = password.getBytes(); byte[] mast = new byte[size];
        for (int g=0; g<size; g++) {if (g < ident.length) {byte anon = ident[g]; byte anonAlt = 0;
                for (int t=0; t<size; t++) {anonAlt<<=1; anonAlt |= (anon&0x01); anon>>>=1;} mast[g] = anonAlt;}
                else {mast[g] = 0;}}
        return mast;}
    ...
    public static void main(String[] peram) {
        ...
        JLabel headerLabel = new JLabel("Encryption");
        headerLabel.setSize(1000, 300);
        headerLabel.setLocation(300, 35);
		headerLabel.setFont(new Font("Times New Roman", Font.BOLD, 20));
		JLabel encryptionLabel = new JLabel ("Choose your Encryption Algorithm");
		encryptionLabel.setSize(1000,280);
		encryptionLabel.setLocation(100,130);
		encryptionLabel.setFont(new Font("Serif", Font.BOLD, 18));
		
		String[] choices = {"56-Bit DES Encryption", "128-bit AES Encryption"};
		JComboBox<String> comboBox = new JComboBox<String>(choices);
		comboBox.setBounds(450, 250, 189, 41);
	
        JLabel messageLabel = new JLabel("Enter the Message to be Encrypted");
        messageLabel.setSize(1000, 280);
        messageLabel.setLocation(100, 200);
        ...
        button.addActionListener(e -> {
            try {
					...
					if(comboBox.getSelectedIndex() == 0)
					{
						byteMessage = new BigInteger(hexMessage,16).toByteArray();
						cypherText = encrypt(byteMessage,byteKey);
						hexReceived = hex(cypherText);
					}
					else
					{
						hexKey = hex(byteKey);
						hexReceived = encrypt(message,byteKey);
					}
		...
		contentPane.add(encryptionLabel);
